  ...
  <div class="h-full relative">
    ...
    <div v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6">
        ...
        :disabled="couponForm.count != 0"
        :error="v.couponForm.reason.$dirty && !v.couponForm.reason.requiredIfReason">
          <template #error>
            <span v-if="v.couponForm.reason.$dirty && !v.couponForm.reason.requiredIfReason">
              Выберите причину невыдачи
            </span>
          </template>
        </Select>
      </div>
      <div v-if="couponForm.list.length > 0" class="w-1/2 mt-8 pt-4 flex flex-col gap-6">
        <h3 class="text-xl font-semibold text-[#000]">
          Купоны
        </h3>
