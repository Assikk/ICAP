...
export default {
  ...
  methods: {
    ...
    initForm() {
      this.snsForm.organization = this.$auth.user.organization
      this.snsForm.user = this.$auth.user.first_name
      this.snsForm.participation_date = dayjs().format('YYYY-MM-DD')
    }
  },
  async mounted() {
    this.initForm()
