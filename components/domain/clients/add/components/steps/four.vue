  ...
  <div class="h-full relative">
    ...
    <div v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6">
        ...
        :error="v.hivForm.hiv_test_result.$dirty && !v.hivForm.hiv_test_result.required"
        @input="selectHivTestResult">
          <template #error>
            <span v-if="v.hivForm.hiv_test_result.$dirty && !v.hivForm.hiv_test_result.required">
              Выберите результат экспресс-теста на ВИЧ
            </span>
          </template>
        </Select>
...
export default {
  ...
  methods: {
    ...
    selectHivTestResult(value) {
      const hiv = this.testresults.filter((item) => item.id == value)[0]
      if(hiv.name == 'Положительный') {
