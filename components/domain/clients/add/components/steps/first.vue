...
<div class="h-full relative">
  ...
  <div class="flex flex-col gap-6" v-else>
    <div class="flex flex-col gap-1">
      <div class="flex items-center gap-6">
        <p class="text-[#374151] font-semibold">
          Согласие на добровольное участие в программе и обработку данных?
        </p>
        <div>
          <CheckBox :list="voluntaryList" textList="name" valueList="value" :value="form.consent_agreement"
          @click="form.consent_agreement = !form.consent_agreement"/>
        </div>
      </div>
      <p v-if="v.form.consent_agreement.$dirty && !v.form.consent_agreement.isTrue"
      class="text-sm font-medium text-[#EF4444]">
        Вы должны принять добровольное участие в программе и обработку данных
      </p>
    </div>
    <h2 class="title">
      Личная информация
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      valueList="id"
      v-model="form.gender"
      :error="v.form.gender.$dirty && !v.form.gender.required">
        <template #error>
          <span v-if="v.form.gender.$dirty && !v.form.gender.required">
            Выберите гендер
          </span>
        </template>
      </Select>
      ...
      <Input whitespace label="Ник в Telegram" v-model="form.messanger"/>
...
export default {
  ...
  methods: {
    ...
    async checkConclusionNum() {
      if(this.form.conclusion_num) {
        if(!this.disabledCheckButtonConclusionNum) {
          ...
          if(res.data.data) {
            this.error_alert(['Такой номер заключения уже имеется'])
          } else this.success_alert('Такой номер заключения свободный')
    ...
    inputPhone() {
      if(!this.form.phone) {
        this.form.preferred_communication_method = null
      }
    },
    findSex() {
