<template>
  ...
  >
    <div
      ...
      @click="show_editSubregion(false)"
    />
    <div
    ...
    >
      <div class="mt-3">
        <div class="flex justify-between pb-8">
          ...
          <div class="cursor-pointer" @click="show_editSubregion(false)">
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_editSubregion(false)">
          Отмена
        </Button>
        <Button medium @click="editSubregion"> Сохранить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      subregion: (state) => state.subregions.subregion,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    ...mapActions({
      edit_subregion: 'subregion/edit_subregion',
      success_alert: 'alert/success_alert',
      error_alert: 'alert/error_alert',
      get_subregion: 'subregion/get_subregion',
    }),
    async editSubregion() {
      if (this.$v.$invalid) {
        this.$v.$touch()
      ...
      try {
        ...
        await this.edit_subregion(editedSubregion)
        this.success_alert('Область успешно изменён')
        this.show_editSubregion(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка редактировании области')
        }
  ...
  async mounted() {
    await this.get_subregion(this.activeSubregion.id)
    this.form = structuredClone(this.activeSubregion)
