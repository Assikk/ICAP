<template>
  ...
  >
    <div
      ...
      @click="show_deletesuborganization(false)"
    />
    <div
    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          ...
          <br>При удаление все данные организации удаляется
          <br>из базы данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deletesuborganization(false)" class="w-full justify-center"> Отмена </Button>
        <Button :disabled="disabledDelete" full @click="deleteSuborganization" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    ...mapActions({
      delete_suborganization: 'suborganization/delete_suborganization',
    ...
    async deleteSuborganization() {
      if (!this.disabledDelete) {
        ...
        try {
          this.loading_alert();
          await this.delete_suborganization(this.activeSuborganization.id);
          this.success_alert('Организация был успешно удален');
        } catch (err) {
          const errors = [];
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удалении Организации');
          }
