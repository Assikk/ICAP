    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Состоял ли участник в ОЗТ?
          <br />При удаление все данные Состоял ли участник в ОЗТ удаляется
          <br />из базы данных
        </p>
      </div>
    ...
    async deleteGender() {
      if (!this.disabledDelete) {
        ...
        try {
          ...
          this.success_alert('Состоял ли участник в ОЗТ был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Состоял ли участник в ОЗТ')
          }
