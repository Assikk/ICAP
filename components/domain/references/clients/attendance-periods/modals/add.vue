<template>
  ...
  >
    <div
      ...
      @click="show_addAttendanceperiods(false)"
    />
    <div
    ...
    >
      <div class="mt-3">
        <div class="flex justify-between pb-8">
          ...
          <div class="cursor-pointer" @click="show_addAttendanceperiods(false)">
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addAttendanceperiods(false)">
          Отмена
        </Button>
        <Button medium @click="addAttendanceperiods"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addAttendanceperiods() {
      if (this.$v.$invalid) {
        this.$v.$touch()
        return
      }
      try {
        await this.create_attendanceperiods({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert('Когда участник состоял на ОЗТ успешно добавлен')
        this.show_addAttendanceperiods(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка при добавлении Когда участник состоял на ОЗТ')
        }
