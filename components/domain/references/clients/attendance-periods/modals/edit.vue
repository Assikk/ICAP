<template>
  ...
  >
    <div
      ...
      @click="show_editAttendanceperiods(false)"
    />
    <div
    ...
    >
      <div class="mt-3">
        <div class="flex justify-between pb-8">
          ...
          <div class="cursor-pointer" @click="show_editAttendanceperiods(false)">
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_editAttendanceperiods(false)">
          Отмена
        </Button>
        <Button medium @click="editAttendanceperiods"> Сохранить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  name: 'AddAttendanceperiodssModal',
  components: {
    Button,
    LoadingDots,
    Input,
  ...
  computed: {
    ...mapState({
      attendanceperiods: (state) => state.attendanceperiodss.attendanceperiods,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...mapMutations({
      show_editAttendanceperiods: 'attendanceperiods/SHOW_EDITATTENDANCEPERIODS',
    ...
    ...mapActions({
      ...
      get_attendanceperiods: 'attendanceperiods/get_attendanceperiodss',
    }),
    async editAttendanceperiods() {
      if (this.$v.$invalid) {
        this.$v.$touch()
        return
      }
      try {
        const editedAttendanceperiods = {
          id: this.form.id,
          form: {
        ...
        await this.edit_attendanceperiods(editedAttendanceperiods)
        this.success_alert('Когда участник состоял на ОЗТ успешно изменён')
        this.show_editAttendanceperiods(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка редактировании Когда участник состоял на ОЗТ')
        }
  ...
  async mounted() {
    await this.get_attendanceperiods(this.activeAttendanceperiods.id)
    this.form = structuredClone(this.activeAttendanceperiods)
