<template>
  ...
  >
    <div
      ...
      @click="show_deletemethoduses(false)"
    />
    <div
    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          ...
          употребления?
          <br />При удаление все данные Преимущественный способ употребления
          удаляется <br />из базы данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deletemethoduses(false)" class="w-full justify-center">
          Отмена
        </Button>
        <Button :disabled="disabledDelete" full @click="deleteMethoduses" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async deleteMethoduses() {
      if (!this.disabledDelete) {
        this.disabledDelete = true
        try {
          this.loading_alert()
          await this.delete_methoduses(this.activeMethoduses.id)
          this.success_alert(
          ...
          if (err.response && err.response.data) {
            ...
            errors.push(
              'Ошибка при удаления Преимущественный способ употребления'
            )
          }
