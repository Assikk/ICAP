    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Рискованное поведение?
          <br />При удаление все данные Рискованное поведениеа удаляется
          <br />из базы данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deleterisk(false)" class="w-full justify-center"> Отмена </Button>
        <Button :disabled="disabledDelete" full @click="deleteRisk" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
  ...
  methods: {
    ...
    async deleteRisk() {
      if (!this.disabledDelete) {
        ...
        try {
          this.loading_alert()
          await this.delete_risk(this.activeRisk.id)
          this.success_alert('Рискованное поведение был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Рискованное поведениеа')
          }
