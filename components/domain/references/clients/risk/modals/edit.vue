  ...
  >
    ...
    >
      <div class="mt-3">
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Редактировать Рискованное поведение
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_editRisk(false)">
        ...
        <div class="grid gap-8 mb-8">
          <div class="flex justify-between gap-x-4">
            <Input
              label="Рискованное поведение (RU)*"
              id="name_ru"
              v-model="form.name_ru"
              placeholder="Введите название Рискованное поведениеа"
            ...
            <Input
              label="Рискованное поведение (KZ)*"
              id="name_kk"
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_editRisk(false)"> Отмена </Button>
        <Button medium @click="editRisk"> Сохранить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async editRisk() {
      ...
      try {
        ...
        await this.edit_risk(editedRisk)
        this.success_alert('Рискованное поведение успешно изменён')
        this.show_editRisk(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка редактировании Рискованное поведениеа')
        }
  ...
  async mounted() {
    await this.get_risk(this.activeRisk.id)
    this.form = structuredClone(this.activeRisk)
