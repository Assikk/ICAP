<template>
  ...
  >
    <div
      ...
      @click="show_deleteresonsreject(false)"
    />
    <div
    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Причины отказа?
          <br />При удаление все данные Причины отказаа удаляется <br />из базы
          данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deleteresonsreject(false)" class="w-full justify-center">
          Отмена
        </Button>
        <Button :disabled="disabledDelete" full @click="deleteResonsreject" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async deleteResonsreject() {
      if (!this.disabledDelete) {
        this.disabledDelete = true
        try {
          this.loading_alert()
          await this.delete_resonsreject(this.activeResonsreject.id)
          this.success_alert('Причины отказа был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Причины отказаа')
          }
