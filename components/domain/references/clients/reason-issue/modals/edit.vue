<template>
  <div class="fixed top-0 bg-[#00000046] left-0 w-full h-full z-10 flex items-center justify-center">
    <div class="absolute top-0 left-0 w-full h-full" @click="show_editReasonissue(false)"/>
    <div class="relative overflow-y-auto bg-white rounded-md pt-6 w-[600px]">
      <div class="flex items-center justify-between gap-4 mb-6 px-6">
        <h3 class="text-lg font-medium leading-6 text-gray-900">
          Изменить Причина невыдачи
        </h3>
        <svg width="20" height="20" fill="#B3B9C9" class="cursor-pointer" @click="show_editReasonissue(false)">
      ...
      <div class="pt-4 my-6 flex justify-end border-t border-[#E5E7EB] px-6">
        <div class="flex gap-4">
          ...
          <Button :disabled="disabledEdit" @click="editReasonissue">
            Сохранить
          </Button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      reasonissue: (state) => state.reasonissue.reasonissue,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    ...mapActions({
      ...
      get_reasonissue: 'reasonissue/get_reasonissue',
    }),
    async editReasonissue() {
      if (this.$v.$invalid) {
        this.$v.$touch()
        return
      }
      if(!this.disabledEdit) {
        ...
        try {
          this.loading_alert()
          await this.edit_reasonissue({
            id: this.activeReasonissue.id,
            form: this.form
          })
          await this.get_reasonissues({page: 1})
          this.success_alert('Причина невыдачи успешно изменён')
          this.show_editReasonissue(false)
        } catch(err) {
          const errors = []
    ...
    async getReasonissue() {
      await this.get_reasonissue(this.activeReasonissue.id)
      for (let keyReasonissue in this.reasonissue) {
        for (let keyForm in this.form) {
          if(keyReasonissue == keyForm) {
            this.form[keyForm] = this.reasonissue[keyReasonissue]
          }
        }
      }
    }
