<template>
  ...
  >
    <div
      ...
      @click="show_deleteconfirmationresult(false)"
    />
    <div
    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Результат подтверждающего теста на
          ВИЧ?
          <br />При удаление все данные Результат подтверждающего теста на ВИЧ
          удаляется <br />из базы данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deleteconfirmationresult(false)" class="w-full justify-center">
          Отмена
        </Button>
        <Button
          :disabled="disabledDelete"
          full
          @click="deleteConfirmationresult"
...
export default {
  ...
  methods: {
    ...
    async deleteConfirmationresult() {
      if (!this.disabledDelete) {
        this.disabledDelete = true
        try {
          this.loading_alert()
          await this.delete_confirmationresult(this.activeConfirmationresult.id)
          this.success_alert(
            'Результат подтверждающего теста на ВИЧ был успешно удален'
          )
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push(
              'Ошибка при удаления Результат подтверждающего теста на ВИЧ'
            )
          }
