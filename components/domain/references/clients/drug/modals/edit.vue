  ...
  >
    ...
    >
      <div class="mt-3">
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Редактировать Вид наркотиков
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_editDrug(false)">
        ...
        <div class="grid gap-8 mb-8">
          <div class="flex justify-between gap-x-4">
            <Input
              label="Вид наркотиков (RU)*"
              id="name_ru"
              v-model="form.name_ru"
              placeholder="Введите название Вид наркотиков"
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_editDrug(false)"> Отмена </Button>
        <Button medium @click="editDrug"> Сохранить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async editDrug() {
      ...
      try {
        ...
        await this.edit_drug(editedDrug)
        this.success_alert('Вид наркотиков успешно изменён')
        this.show_editDrug(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка редактировании Вид наркотиков')
        }
  ...
  async mounted() {
    await this.get_drug(this.activeDrug.id)
    this.form = structuredClone(this.activeDrug)
