    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Вид наркотиков?
          <br />При удаление все данные Вид наркотиков удаляется <br />из базы
          данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deletedrug(false)" class="w-full justify-center"> Отмена </Button>
        <Button :disabled="disabledDelete" full @click="deleteDrug" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
  ...
  methods: {
    ...
    async deleteDrug() {
      if (!this.disabledDelete) {
        ...
        try {
          this.loading_alert()
          await this.delete_drug(this.activeDrug.id)
          this.success_alert('Вид наркотиков был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Вид наркотиков')
          }
