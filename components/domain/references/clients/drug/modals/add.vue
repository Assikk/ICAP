  ...
  >
    ...
    >
        ...
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Добавить Вид наркотиков
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_addDrug(false)">
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addDrug(false)"> Отмена </Button>
        <Button medium @click="addDrug"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addDrug() {
      ...
      try {
        await this.create_drug({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert('Вид наркотиков успешно добавлен')
        this.show_addDrug(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка при добавлении Вид наркотиков')
        }
