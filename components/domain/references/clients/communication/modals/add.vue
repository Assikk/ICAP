<template>
  ...
  >
    <div
      ...
      @click="show_addCommunication(false)"
    />
    <div
    ...
    >
      <div class="mt-3">
        <div class="flex justify-between pb-8">
          <div>
            ...
            >
              Добавить Предпочитаемый метод связи
            </h3>
          </div>
          <svg width="20" height="20" fill="#B3B9C9" class="cursor-pointer" @click="show_addCommunication(false)">
        ...
        <div class="grid gap-8 mb-8">
          <div class="flex justify-between gap-x-4">
            <Input
              label="Предпочитаемый метод связи (RU)*"
              id="name_ru"
              v-model="name_ru"
            ...
            <Input
              label="Предпочитаемый метод связи (KZ)*"
              id="name_kk"
              v-model="name_kk"
              placeholder="Введите Предпочитаемый метод связи"
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addCommunication(false)">
          Отмена
        </Button>
        <Button medium @click="addCommunication"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addCommunication() {
      ...
      try {
        await this.create_communication({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert('Предпочитаемый метод связи успешно добавлен')
        this.show_addCommunication(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка при добавлении Предпочитаемый метод связи')
        }
