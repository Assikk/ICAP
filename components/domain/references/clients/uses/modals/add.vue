  ...
  >
    ...
    >
        ...
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Добавить Преимущественный способ употребления
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_addUses(false)">
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addUses(false)"> Отмена </Button>
        <Button medium @click="addUses"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addUses() {
      ...
      try {
        await this.create_uses({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert(
          'Преимущественный способ употребления успешно добавлен'
        )
        this.show_addUses(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push(
            'Ошибка при добавлении Преимущественный способ употребленияа'
          )
        }
