<template>
  <div class="fixed top-0 bg-[#00000046] left-0 w-full h-full z-10 flex items-center justify-center">
    <div class="absolute top-0 left-0 w-full h-full" @click="show_editCategorykgn(false)"/>
    <div class="relative overflow-y-auto bg-white rounded-md pt-6 w-[600px]">
      <div class="flex items-center justify-between gap-4 mb-6 px-6">
        <h3 class="text-lg font-medium leading-6 text-gray-900">
          Редактировать Категория КГН
        </h3>
        <svg width="20" height="20" fill="#B3B9C9" class="cursor-pointer" @click="show_editCategorykgn(false)">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-4 px-6">
        <Input label="Категория КГН (RU)*" id="name_ru" v-model="form.name_ru"
        ...
        <Input label="Категория КГН (KZ)*" id="name_kk" v-model="form.name_kk"
          placeholder="Введите название Категория КГН"
      ...
      <div class="pt-4 my-6 flex justify-end border-t border-[#E5E7EB] px-6">
        <div class="flex gap-4">
          <Button white @click="show_editCategorykgn(false)">
            Отмена
          </Button>
          <Button :disabled="disabledEdit" @click="editKgn">
            Сохранить
          </Button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      categorykgn: (state) => state.categorykgn.categorykgn,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    ...mapActions({
      ...
      get_categorykgn: 'categorykgn/get_categorykgn'
    }),
    async editKgn() {
      if (this.$v.$invalid) {
        this.$v.$touch()
      ...
      if(!this.disabledEdit) {
        ...
        try {
          this.loading_alert()
          await this.edit_categorykgn({
            id: this.activeCategorykgn.id,
            form: this.form
          })
          await this.get_categorykgns({page: this.activePage})
          this.success_alert('Категория КГН успешно изменён')
          this.show_editCategorykgn(false)
        } catch(err) {
          const errors = []
    ...
    async getCategoryKgn() {
      await this.get_categorykgn(this.activeCategorykgn.id)
      for (let keyCategoryKgn in this.categorykgn) {
        for (let keyForm in this.form) {
          if(keyCategoryKgn == keyForm) {
            this.form[keyForm] = this.categorykgn[keyCategoryKgn]
          }
        }
      }
    }
