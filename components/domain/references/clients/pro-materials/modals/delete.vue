<template>
  ...
  >
    <div
      ...
      @click="show_deletepromaterials(false)"
    />
    <div
    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Набор проф. материалов?
          <br />При удаление все данные Набор проф. материалова удаляется
          <br />из базы данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deletepromaterials(false)" class="w-full justify-center">
          Отмена
        </Button>
        <Button :disabled="disabledDelete" full @click="deletePromaterials" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async deletePromaterials() {
      if (!this.disabledDelete) {
        this.disabledDelete = true
        try {
          this.loading_alert()
          await this.delete_promaterials(this.activePromaterials.id)
          this.success_alert('Набор проф. материалов был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Набор проф. материалова')
          }
