<template>
  ...
  >
    <div
      ...
      @click="show_editPromaterials(false)"
    />
    <div
    ...
    >
      <div class="mt-3">
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Редактировать Набор проф. материалов
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_editPromaterials(false)">
        ...
        <div class="grid gap-8 mb-8">
          <div class="flex justify-between gap-x-4">
            <Input
              label="Набор проф. материалов (RU)*"
              id="name_ru"
              v-model="form.name_ru"
              placeholder="Введите название Набор проф. материалова"
            ...
            <Input
              label="Набор проф. материалов (KZ)*"
              id="name_kk"
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_editPromaterials(false)">
          Отмена
        </Button>
        <Button medium @click="editPromaterials"> Сохранить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      promaterials: (state) => state.promaterialss.promaterials,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    ...mapActions({
      ...
      get_promaterials: 'promaterials/get_promaterialss',
    }),
    async editPromaterials() {
      if (this.$v.$invalid) {
        this.$v.$touch()
        return
      }
      try {
        const editedPromaterials = {
          id: this.form.id,
          form: {
        ...
        await this.edit_promaterials(editedPromaterials)
        this.success_alert('Набор проф. материалов успешно изменён')
        this.show_editPromaterials(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка редактировании Набор проф. материалова')
        }
  ...
  async mounted() {
    await this.get_promaterials(this.activePromaterials.id)
    this.form = structuredClone(this.activePromaterials)
