<template>
  ...
  >
    <div
      ...
      @click="show_addPromaterials(false)"
    />
    <div
    ...
    >
        ...
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Добавить Набор проф. материалов
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_addPromaterials(false)">
        ...
        <div class="grid gap-8 mb-8">
          <div class="flex justify-between gap-x-4">
            <Input
              ...
              id="name_ru"
              v-model="name_ru"
              placeholder="Введите название Набор проф. материалов"
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addPromaterials(false)">
          Отмена
        </Button>
        <Button medium @click="addPromaterials"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addPromaterials() {
      if (this.$v.$invalid) {
        this.$v.$touch()
        return
      }
      try {
        await this.create_promaterials({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert('Набор проф. материалов успешно добавлен')
        this.show_addPromaterials(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка при добавлении Набор проф. материалова')
        }
