<template>
  ...
  >
    <div
      ...
      @click="show_addQuestionnaire(false)"
    />
    <div
    ...
    >
        ...
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Добавить Тип опросника
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_addQuestionnaire(false)">
        ...
        <div class="grid gap-8 mb-8">
          <div class="flex justify-between gap-x-4">
            <Input
              ...
              id="name_ru"
              v-model="name_ru"
              placeholder="Введите название Тип опросника"
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addQuestionnaire(false)">
          Отмена
        </Button>
        <Button medium @click="addQuestionnaire"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addQuestionnaire() {
      ...
      try {
        await this.create_questionnaire({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert('Тип опросника успешно добавлен')
        this.show_addQuestionnaire(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка при добавлении Тип опросникаа')
        }
