<template>
  ...
  >
    <div
      ...
      @click="show_deletequestionnaire(false)"
    />
    <div
    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Тип опросника?
          <br />При удаление все данные Тип опросникаа удаляется <br />из базы
          данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deletequestionnaire(false)" class="w-full justify-center">
          Отмена
        </Button>
        <Button :disabled="disabledDelete" full @click="deleteQuestionnaire" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
  ...
  methods: {
    ...
    async deleteQuestionnaire() {
      if (!this.disabledDelete) {
        ...
        try {
          this.loading_alert()
          await this.delete_questionnaire(this.activeQuestionnaire.id)
          this.success_alert('Тип опросника был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Тип опросникаа')
          }
