<template>
  ...
  >
    <div
      ...
      @click="show_deletetestresult(false)"
    />
    <div
    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Результат экспресс-теста на ВИЧ?
          <br />При удаление все данные Результат экспресс-теста на ВИЧа
          удаляется <br />из базы данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deletetestresult(false)" class="w-full justify-center">
          Отмена
        </Button>
        <Button :disabled="disabledDelete" full @click="deleteTestresult" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
  ...
  methods: {
    ...
    async deleteTestresult() {
      if (!this.disabledDelete) {
        ...
        try {
          this.loading_alert()
          await this.delete_testresult(this.activeTestresult.id)
          this.success_alert(
            'Результат экспресс-теста на ВИЧ был успешно удален'
          )
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Результат экспресс-теста на ВИЧа')
          }
