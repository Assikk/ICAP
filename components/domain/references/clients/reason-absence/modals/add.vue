<template>
  ...
  >
    <div
      ...
      @click="show_addReasonabsence(false)"
    />
    <div
    ...
    >
        ...
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Добавить Причина отсутствия подтверждающего теста
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_addReasonabsence(false)">
        ...
        <div class="grid gap-8 mb-8">
          <div class="flex justify-between gap-x-4">
            <Input
              ...
              id="name_ru"
              v-model="name_ru"
              placeholder="Введите название Причина отсутствия подтверждающего теста"
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addReasonabsence(false)">
          Отмена
        </Button>
        <Button medium @click="addReasonabsence"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addReasonabsence() {
      if (this.$v.$invalid) {
        this.$v.$touch()
        return
      }
      try {
        await this.create_reasonabsence({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert(
          'Причина отсутствия подтверждающего теста успешно добавлен'
        )
        this.show_addReasonabsence(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push(
            'Ошибка при добавлении Причина отсутствия подтверждающего тестаа'
          )
        }
