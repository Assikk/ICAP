<template>
  ...
  >
    <div
      ...
      @click="show_addParticipantiom(false)"
    />
    <div
    ...
    >
        ...
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Добавить Участнику выданы ИОМ
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_addParticipantiom(false)">
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addParticipantiom(false)">
          Отмена
        </Button>
        <Button medium @click="addParticipantiom"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addParticipantiom() {
      if (this.$v.$invalid) {
        this.$v.$touch()
        return
      }
      try {
        await this.create_participantiom({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert('Участнику выданы ИОМ успешно добавлен')
        this.show_addParticipantiom(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка при добавлении Участнику выданы ИОМ')
        }
