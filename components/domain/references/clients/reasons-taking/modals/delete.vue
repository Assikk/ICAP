<template>
  ...
  >
    <div
      ...
      @click="show_deletereasonstaking(false)"
    />
    <div
    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Причины не взятия на АРТ?
          <br />При удаление все данные Причины не взятия на АРТа удаляется
          <br />из базы данных
        </p>
      </div>
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full @click="show_deletereasonstaking(false)" class="w-full justify-center">
          Отмена
        </Button>
        <Button :disabled="disabledDelete" full @click="deleteReasonstaking" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async deleteReasonstaking() {
      if (!this.disabledDelete) {
        this.disabledDelete = true
        try {
          this.loading_alert()
          await this.delete_reasonstaking(this.activeReasonstaking.id)
          this.success_alert('Причины не взятия на АРТ был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Причины не взятия на АРТа')
          }
