<template>
  ...
  >
    <div
      ...
      @click="show_addReasonstaking(false)"
    />
    <div
    ...
    >
        ...
        <div class="flex justify-between pb-8">
          <div>
            <h3
              ...
              id="modal-title"
            >
              Добавить Причины не взятия на АРТ
            </h3>
          </div>
          <div class="cursor-pointer" @click="show_addReasonstaking(false)">
        ...
        <div class="grid gap-8 mb-8">
          <div class="flex justify-between gap-x-4">
            <Input
              ...
              id="name_ru"
              v-model="name_ru"
              placeholder="Введите название Причины не взятия на АРТ"
      ...
      <div class="flex gap-4 justify-end py-3 border-t border-t-[#E5E7EB]">
        <Button white medium @click="show_addReasonstaking(false)">
          Отмена
        </Button>
        <Button medium @click="addReasonstaking"> Добавить </Button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addReasonstaking() {
      if (this.$v.$invalid) {
        this.$v.$touch()
        return
      }
      try {
        await this.create_reasonstaking({
          name_ru: this.name_ru,
          name_kk: this.name_kk,
        })
        this.success_alert('Причины не взятия на АРТ успешно добавлен')
        this.show_addReasonstaking(false)
      } catch (err) {
        const errors = []
        if (err.response?.data) {
          ...
          errors.push('Ошибка при добавлении Причины не взятия на АРТа')
        }
