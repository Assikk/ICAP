    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот гендер?
          <br />При удаление все данные Количество выданных купонов удаляется
          <br />из базы данных
        </p>
      </div>
    ...
    async deleteGender() {
      if (!this.disabledDelete) {
        ...
        try {
          ...
          this.success_alert('Гендер был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Количество выданных купонов')
          }
