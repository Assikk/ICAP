    ...
    >
      <div>
        ...
        <p class="text-sm text-[#5E6470] mb-4">
          Вы уверены что хотите удалить этот Участнику предложена ДКП?
          <br />При удаление все данные Участнику предложена ДКП удаляется
          <br />из базы данных
        </p>
      </div>
    ...
    async deleteGender() {
      if (!this.disabledDelete) {
        ...
        try {
          ...
          this.success_alert('Участнику предложена ДКП был успешно удален')
        } catch (err) {
          const errors = []
          if (err.response && err.response.data) {
            ...
            errors.push('Ошибка при удаления Участнику предложена ДКП')
          }
