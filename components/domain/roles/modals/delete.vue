<template>
  <div class="fixed top-0 bg-[#00000046] left-0 w-full h-full z-10 flex items-center justify-center">
    <div class="absolute top-0 left-0 w-full h-full" @click="show_deleteRole(false)"/>
    <div class="relative bg-white text-center rounded-md px-6 py-4 flex flex-col gap-4 w-96">
      ...
      <div class="flex gap-3 py-3 px-8 border-t border-t-[#E5E7EB]">
        <Button white full class="w-full justify-center" @click="show_deleteRole(false)"> Отмена </Button>
        <Button :disabled="disabledDelete" full @click="deleteRole" class="w-full justify-center">
          Да
        </Button>
      </div>
    </div>
  ...
  methods: {
    ...
    async deleteRole() {
      if(!this.disabledDelete) {
        ...
        try {
          this.loading_alert()
          await this.delete_role(this.activeRole.id)
          await this.get_roles({page: this.activePage})
          this.success_alert('Роль успешно удален')
          this.show_deleteRole(false)
        } catch(err) {
          const errors = []
